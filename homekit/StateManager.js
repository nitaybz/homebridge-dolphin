	...
	return {
		...
		set: {
			TargetHeatingCoolingState: (state) => {
				if (!state) {
					...
					return dolphinApi.turnOff(device.deviceName)
						.finally(updateState)
				} else if (deviceCache.targetTemperature && deviceCache.targetTemperature < deviceCache.Temperature) {
					log.easyDebug(`Turning OFF Device ${device.deviceName} - Target Temperature is Lower than Current Temperature`)
					return dolphinApi.turnOff(device.deviceName)
						.finally(updateState)
				} else {
			...
			TargetTemperature: (temp) => {
				if (temp < deviceCache.Temperature) {
					log.easyDebug(`Turning OFF Device ${device.deviceName} - Target Temperature is Lower than Current Temperature`)
					return dolphinApi.turnOff(device.deviceName)
						.finally(updateState)
				}
